  ...
  return (
    <AccountLayout>
      <div className="rounded-xl border border-[var(--voya-border)] p-6" style={{ backgroundColor: "rgba(255,255,255,0.03)" }}>
        ...
        <p className="mb-5 text-xs text-[var(--voya-text-3)]">Your rights of access, rectification, erasure and portability are all actionable from this screen.</p>
        ...
        <div className="space-y-4">
          {[
            ...
            <div key={item.label} className="flex items-start justify-between gap-4 border-b border-[var(--voya-border)] pb-4">
              <div>
                <p className="text-sm font-medium text-white">{item.label}</p>
                <p className="text-xs text-[var(--voya-text-3)]">{item.desc}</p>
              </div>
              {item.btnHref ? (
                <Link href={item.btnHref} className="rounded border border-[var(--voya-border)] px-3 py-1.5 text-xs text-[var(--voya-text-2)] hover:text-[var(--voya-text)] transition-colors whitespace-nowrap">
                  {item.btn}
                </Link>
              ) : (
                <button
                  ...
                  className="rounded border border-[var(--voya-border)] px-3 py-1.5 text-xs text-[var(--voya-text-2)] hover:text-[var(--voya-text)] transition-colors whitespace-nowrap"
          ...
          <div className="flex items-start justify-between gap-4 pb-4">
            <div>
              ...
              <p className="text-xs text-[var(--voya-text-3)]">Named journey events only — never your name, email, date of birth or document numbers.</p>
            </div>
            ...
            <tbody>
              {RETENTION.map(r => (
                <tr key={r.category} className="border-b border-white/6">
                  <td className="py-2.5 text-sm text-[var(--voya-text-2)]">{r.category}</td>
                  <td className="py-2.5 text-sm text-[var(--voya-text-3)]">{r.classification}</td>
                  <td className="py-2.5 text-sm text-[var(--voya-text-3)]">{r.retention}</td>
                </tr>
              ))}
            </tbody>
          </table>
          <p className="mt-2 text-xs text-[var(--voya-text-4)]">Deletion is a cryptographic erasure, not a hidden record.</p>
        </div>
      </div>
    </AccountLayout>
  );
