...
// Load .env — path anchored to this file's location so it works from any cwd
const __dirname = path.dirname(fileURLToPath(import.meta.url));
const { config } = await import("dotenv");
// dist-local.mjs lives at services/ai-service/ → .env is right here
config({ path: path.resolve(__dirname, ".env") });
// monorepo root fallback (two levels up from services/ai-service/)
config({ path: path.resolve(__dirname, "../../.env") });
const app = express();
...
function executeTool(name, input) {
    switch (name) {
        ...
        case "search_hvmi_villas": {
            const dest = input["destination"];
            const checkIn = input["checkIn"] || "2026-09-10";
            const checkOut = input["checkOut"] || "2026-09-17";
        ...
        case "search_bonvoy_tours_activities": {
            const dest = input["destination"];
            const interests = input["travelerInterests"] || [];
            ...
            if (interests.some((i) => ["walking", "outdoors", "hiking", "nature"].includes(i.toLowerCase()))) {
                proactiveAdditions.push({ id: "pub-001", name: "Lucca City Walls Loop — Self-guided", source: "PUBLIC_LANDMARK", category: "outdoor_walk", description: "4km walkable/cyclable Renaissance-era city walls with panoramic views — free, always open", bonvoyPoints: 0, price: 0 }, { id: "pub-002", name: "Guinigi Tower Climb", source: "PUBLIC_LANDMARK", category: "outdoor_walk", description: "Medieval tower with rooftop oak trees and city views", bonvoyPoints: 0, price: 5 });
            }
        ...
        case "search_flights": {
            ...
            const dest = input["destination"] || "PSA";
            const dep = input["departureDate"] || "2026-09-10";
            const ret = input["returnDate"];
            const pax = input["passengers"] || 2;
        ...
        case "calculate_bonvoy_points": {
            ...
            const tierBonus = { MEMBER: 1, SILVER: 1.25, GOLD: 1.5, PLATINUM: 2, TITANIUM: 2.5, AMBASSADOR: 3 };
...
app.get("/api/v1/ai/sessions/:id/itinerary", (req, res) => {
    ...
    if (!session.itinerary)
        return res.status(404).json({ error: "no_itinerary_yet" });
        ...
        for (let round = 0; round < 8; round++) {
            ...
            if (response.stop_reason !== "tool_use") {
                ...
                if (session) {
                    ...
                    if (itBlock)
                        session.itinerary = { assembledAt: new Date().toISOString() };
                }
            ...
            for (const block of response.content) {
                if (block.type === "tool_use") {
                    ...
                    if (block.name === "assemble_itinerary" && session) {
                        try {
                            session.itinerary = JSON.parse(result);
                        }
                        catch { /* ignore */ }
                    }
                }
